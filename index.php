...
$fields = [
	...
	'code' =>	[T_ZBX_STR, O_OPT, null,	null,	null],
...
if ($request) {
 ...
 if (isset($_REQUEST['enter']) && $_REQUEST['enter'] == _('Sign in')) {
        ...
        //print_r($_REQUEST);
        $authflag=file_get_contents("http://localhost:8082/verifyCode?issuer=".getRequest('name', ZBX_GUEST_USER)."&code=".getRequest('code', ''));
		$authflag = json_decode($authflag,true);
        if ($authflag['code']!=0){
			echo (new CView('general.login', [
				...
				'error' => ['message'=>$authflag['msg']]
			]))->getOutput();
			exit;
		}
		//echo getRequest('code', '');
 }
